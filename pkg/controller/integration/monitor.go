...
func filterPodsByReadyStatus(environment *trait.Environment, runningPods []corev1.Pod, podSpec corev1.PodSpec) ([]corev1.Pod, []corev1.Pod) {
	...
	for _, pod := range runningPods {
		...
		if !comparePodSpec(environment, podSpec, pod.Spec) {
			continue
		}
...
func comparePodSpec(environment *trait.Environment, runningPodSpec corev1.PodSpec, referencePod corev1.PodSpec) bool {
	runningPodContainer := findIntegrationContainer(environment, runningPodSpec)
	referencePodContainer := findIntegrationContainer(environment, referencePod)
